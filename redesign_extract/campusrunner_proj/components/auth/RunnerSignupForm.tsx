...
export function RunnerSignupForm() {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="mb-2 text-center"><div className="mx-auto mb-4 w-fit"><BrandMark className="h-14 w-14" /></div><h1 className="text-2xl font-black">Create runner account</h1><p className="mt-2 text-sm text-slate-500">Accept jobs and earn on campus.</p></div>
      ...
      <button disabled={loading} className="w-full rounded-2xl bg-[linear-gradient(135deg,#6200EE,#4F2EE8)] px-5 py-3.5 font-bold text-white shadow-lg shadow-violet-500/20 disabled:opacity-60">{loading ? 'Creating account...' : 'Continue as Runner'}</button>
