...
export function RunnerSignupForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: runnerError } = await supabase.from('runners').insert([
        {
          profile_id: authData.user.id,
          student_id_number: formData.matric_number,
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="auth-alert success">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <strong>Runner account created.</strong> We’ll notify you after verification.
          </div>
        </div>
      </motion.div>
    );
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 auth-animate">
      <div className="auth-card-header auth-animate auth-delay-1">
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 8 }}>
          <Zap className="auth-icon" />
        </div>
        <h1 className="auth-title">Become a runner</h1>
        <p className="auth-sub">Start earning on campus with verified requests.</p>
      </div>
