    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.back()}
            className="px-4 py-2 bg-amber-500 text-white rounded-xl font-semibold hover:bg-amber-600"
          >
            Go Back
          </button>
        </div>
  ...
  return (
    <div className="flex-1 p-6 lg:p-8 ">
      ...
      >
        ...
        <p className="text-slate-500 mt-2">Order ID: {order.id}</p>
      </motion.div>
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-slate-500 mt-1">{order.description}</p>
              </div>
            ...
            <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-6">
              ...
              <div className="space-y-3">
                {order.order_items.map((item) => (
                  <div key={item.id} className="flex justify-between items-center py-2 border-b border-slate-200/80 last:border-b-0">
                    <div>
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-slate-900">Order Created</p>
              ...
              {order.updated_at !== order.created_at && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{new Date(order.updated_at).toLocaleString()}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-900">₦{(order.platform_fee || 0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Runner Payout:</span>
                <span className="font-semibold text-slate-900">₦{(order.runner_payout || 0).toLocaleString()}</span>
              </div>
            </div>
          </div>

          <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-6">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Student</h2>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <User className="h-4 w-4 text-slate-500" />
                <span className="font-semibold text-slate-900">{order.student_profile?.full_name}</span>
              </div>
              <p className="text-sm text-slate-500">{order.student_profile?.email}</p>
              <p className="text-sm text-slate-500">{order.student_profile?.phone}</p>
            </div>
          </div>

          {order.runner_profile && (
            <div className="rounded-2xl border border-slate-200/80 bg-white/80 p-6">
              <h2 className="text-xl font-bold text-slate-900 mb-4">Runner</h2>
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <User className="h-4 w-4 text-slate-500" />
                  <span className="font-semibold text-slate-900">{order.runner_profile.full_name}</span>
                </div>
                <p className="text-sm text-slate-500">{order.runner_profile.email}</p>
                <p className="text-sm text-slate-500">{order.runner_profile.phone}</p>
              </div>
            </div>
          )}
        </motion.div>
