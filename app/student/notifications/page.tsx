...
export default function StudentNotificationsPage() {
  ...
  function getIcon(title: string) {
    if (title.includes('Accepted') || title.includes('assigned')) return '✅';
    if (title.includes('Progress') || title.includes('way'))      return '🚀';
    if (title.includes('Completed') || title.includes('Delivered')) return '📦';
    if (title.includes('Cancelled')) return '❌';
    return '🔔';
  }

  function getIconBg(title: string) {
    if (title.includes('Accepted') || title.includes('assigned')) return 'rgba(22,163,74,.1)';
    if (title.includes('Progress') || title.includes('way'))      return 'rgba(217,119,6,.1)';
    if (title.includes('Completed') || title.includes('Delivered')) return 'rgba(15,61,46,.1)';
    if (title.includes('Cancelled')) return 'rgba(220,38,38,.08)';
      ...
      {filtered.length === 0 ? (
        <div className="sd-card" style={{ textAlign: 'center', padding: '48px 24px' }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--ink3)' }}>
            {filter === 'unread' ? 'No unread notifications' : "You'll be notified about order updates here"}
          </div>
        </div>
      ) : (
        <div className="sd-card" style={{ padding: 0, overflow: 'hidden' }}>
          {filtered.map((n, i) => (
            ...
            >
              ...
              {!n.is_read && (
                <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--gold-d)', flexShrink: 0, marginTop: 2 }}>Tap to read</div>
              )}
            </div>
          ))}
        </div>
