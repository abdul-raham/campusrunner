...
export default function RunnerProfilePage() {
  ...
  const fetchStats = async () => {
    try {
      const [statusRes, { data: orders }] = await Promise.all([
        fetch('/api/runner/status').then(r => r.ok ? r.json() : null),
        supabase.from('orders').select('final_amount,platform_fee,completed_at').eq('runner_id', user?.id).eq('status', 'completed'),
      ]);
      if (statusRes) setRunnerStats(statusRes);
      if (orders) {
        const now = new Date();
        const total = orders.reduce((s, o) => s + ((o.final_amount || 0) - (o.platform_fee || 0)), 0);
        const month = orders
          .filter(o => o.completed_at && new Date(o.completed_at).getMonth() === now.getMonth())
          .reduce((s, o) => s + ((o.final_amount || 0) - (o.platform_fee || 0)), 0);
        setEarnings({ total, month });
      }
  ...
  return (
    <div className="sd-content" style={{ animation: 'fadeIn .4s ease both', maxWidth: 640, margin: '0 auto' }}>
      ...
      <div style={{ borderRadius: 24, overflow: 'hidden', background: 'rgba(255,255,255,.85)', backdropFilter: 'blur(20px) saturate(160%)', WebkitBackdropFilter: 'blur(20px) saturate(160%)', border: '1px solid rgba(0,0,0,.08)', boxShadow: '0 2px 8px rgba(0,0,0,.06),0 8px 24px rgba(0,0,0,.05)' }}>
        ...
        <div style={{ padding: '0 24px 28px' }}>
          ...
          <div style={{ marginTop: 14 }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: 'var(--ink)', letterSpacing: '-.02em' }}>{form.full_name || 'Runner'}</div>
          ...
          {/* Stats */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 0, marginTop: 20, paddingTop: 20, borderTop: '1px solid var(--bdr)' }}>
            {[
              { val: runnerStats.rating > 0 ? runnerStats.rating.toFixed(1) : '—', lbl: 'Rating' },
              { val: runnerStats.total_jobs,                                         lbl: 'Jobs Done' },
              { val: `₦${(earnings.month / 1000).toFixed(1)}k`,                     lbl: 'This Month' },
              { val: `₦${(earnings.total / 1000).toFixed(1)}k`,                     lbl: 'All Time' },
            ].map((s, i, arr) => (
              <div key={s.lbl} style={{ textAlign: 'center', padding: '8px 0', borderRight: i < arr.length - 1 ? '1px solid var(--bdr)' : 'none' }}>
                <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--ink)', letterSpacing: '-.03em' }}>{s.val}</div>
                <div style={{ fontSize: 10, color: 'var(--ink3)', marginTop: 3 }}>{s.lbl}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ borderRadius: 20, overflow: 'hidden', background: 'rgba(255,255,255,.85)', backdropFilter: 'blur(20px) saturate(160%)', WebkitBackdropFilter: 'blur(20px) saturate(160%)', border: '1px solid rgba(0,0,0,.08)', boxShadow: '0 2px 8px rgba(0,0,0,.06),0 8px 24px rgba(0,0,0,.05)' }}>
        {[
          { icon: '💼', label: 'Browse Jobs',   sub: 'Find new requests',    href: '/runner/jobs' },
          { icon: '🏃', label: 'Active Jobs',   sub: 'Manage current jobs',  href: '/runner/jobs/active' },
          { icon: '💰', label: 'Earnings',      sub: 'View your income',     href: '/runner/earnings' },
